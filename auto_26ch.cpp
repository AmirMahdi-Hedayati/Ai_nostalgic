...
string pop[NN]; // save the strings

int ans[NN]; // save the answers of user

int poss[NN][27]; // save possibility of each character in each place 
...
void reset_poss()
{
	for(int i = 0 ; i < NN ; i++)
	{
		...
		for (int j = 0; j < 26; j++) poss[i][j] = 1000;
	}	
}
...
string edgham(string g1 , string g2 )
{
	...
	for(int i = 0; i < NN; i++)
	{
		...
		else if( (g1.at(i)) != (g2.at(i)) && p_rand(70) ) o.push_back (crand(i));
...
void check()
{
	string ramz = "AMIRMAHDIHEDAYATIPOORARAKZ" ;
	int an;
...
int main()
{
	...
	while(finish())
	{
		...
		for(int i = 0; i < NN; i++) // finding first and second maximom answers of user (mx1,mx2) and find the best strings to make next generation
		{
			if (ans[i] > mx1)
			{
				mx2 = mx1;
